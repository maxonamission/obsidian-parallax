...
**The sidebar outline shows every section.** A session note can carry up to seventeen sections; the Artifacts list showed eight of them. It now lists every section that exists, in the note's canonical order, with References at the end — and when an assistant lands a new section, the note scrolls to it. A section you inserted but haven't written yet no longer counts as done in the workflow strip.

**Three ways to begin, one verb.** The start commands are now *Project · start (folder + first question)*, *Question · start in this project (new session note)* and *Question · start in this note (turn it into a session)*. Command ids and hotkeys are unchanged. Starting a session without a selection proposes the note's title as the question, and the "Add to a new session note" button says which project that note will land in.

**Feedback has a place now.** Parallax collects no usage data and never will; instead there is a *Feedback or a bug?* link in the settings (Advanced) and in the sidebar's welcome panel, and the public repository has issue templates. On a phone, creating a project now opens the new note in the main area rather than in the sidebar you tapped in — the same class of bug as 0.31.1.

**The sidebar now starts where your thinking starts.** At the very top there is now a single entry block — your objective and your question, and one button that fits the note you have open: *Start research project* on an empty note, *Start question in this note* on a plain one, *New question in this project* on a project hub, and **Edit question** once the note is a session (with one dated logbook line when you actually change the wording). Naming the problem was scattered over three places before — a welcome panel, two buttons in the project panel and a card in the steps list — and none of them let you change the question afterwards. The same change reaches the command palette: the two *Question · start…* commands now appear only where they apply, so you see one of them rather than both and never the one that would fail. Worth knowing: a command that is hidden does not fire from its hotkey either, so a shortcut for *Question · start in this project* does nothing outside a project. Command ids are untouched, so the hotkeys you set still work where the command applies.

**Every phase has its cards.** The workflow strip showed six phases while the steps list had cards for five, and eight of the Design commands lived in the palette only. Now: **Frame · framing** lets you write the framing yourself, in one field, with no AI involved — until now the only way to choose a framing was to run a problem exploration; **Frame · framework** moved up out of the collapsed More group; confronting your beliefs is **Evidence · confront beliefs**, because it reasons against the synthesis rather than against your framing; and Design has four cards — argument map, hypotheses, interview guide and research agenda — which also means the *Basis changed* badge finally reaches them when something they were built on has been revised. A new **Wrap up** block gathers the methodological account and the exports (session, project, project bibliography): deterministic, on-demand work, deliberately not a seventh phase. And the artefact outline now carries the colour of the phase that wrote each section, which makes one deliberate difference readable: the strip is the order you *think* in, the note is the order it *reads* in, which is why the challenge sits after the synthesis in your note.

**The project index keeps up with renames.** The hub's Contents list already refreshed itself whenever a note was created in the project folder; now it also follows a note you rename or move, in both the old and the new project. Deleting stays a deliberate step, because a deletion can also be a sync hiccup or an undo, so the Project block in the sidebar now carries a small *Refresh contents* link for exactly that case.

**Fewer things called "question".** The literature run is now *Evidence · run research* (card: *Literature research*), so the word "question" is reserved for the two commands that set a session's research question. And the Framework card gained *Add section*, like every other thinking step: write the construct, working definition and dimensions yourself, no AI needed.

**A process map, on one page.** The README now opens with a map of every step: what it does, where the result lands, and how the sidebar offers it. Useful when you want to see the whole method before you commit to a run, and as a reminder that the strip is the thinking order while the note is the reading order.

*Under the hood, no behaviour change:* the assistant flows moved out of the plugin's main file into three controllers with tests (coverage rose from 60 to 67 percent, now enforced as a floor in CI), all notices go through one route, the LLM transports share one retry helper, and the provider list is generated from a single descriptor table. The debug-logging setting now says exactly what the debug note contains and that it lives in your vault like any other note.

*What we would like to hear from you:* does the entry block put the right thing first — is naming the objective and the question where you want to start, and does the one button it offers match what you meant to do? And do the new cards cover the steps you actually take, or is there one you still go hunting for in the palette? Tell us at [github.com/maxonamission/obsidian-parallax/issues](https://github.com/maxonamission/obsidian-parallax/issues).
